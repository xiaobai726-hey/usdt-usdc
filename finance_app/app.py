...
def show_dashboard():
    ...
    # Fetch data
    budget = db.get_budget(selected_month)
    ...
    col1.metric("本月总收入", f"¥{income:,.2f}", f"预算: ¥{budget['income_budget']:,.2f}")
    col2.metric("本月总支出", f"¥{expense:,.2f}", f"预算: ¥{budget['expense_budget']:,.2f}")
    col3.metric("剩余支出预算", f"¥{remaining_budget:,.2f}")
    col4.metric("净收益", f"¥{(income - expense):,.2f}")
...
def show_upload():
    ...
    uploaded_file = st.file_uploader("选择CSV账单文件", type=['csv'])
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            source = st.text_input("账单来源 (如: 招商银行, 支付宝)", "Bank")
        ...
        if st.button("处理并导入"):
            with st.spinner("处理中..."):
                processed_df, error = parser.process_statement(
...
def show_settings():
    ...
    current_budget = db.get_budget(month_input)
    
    col1, col2 = st.columns(2)
    with col1:
        income_budget = st.number_input("预期收入 (¥)", min_value=0.0, value=float(current_budget['income_budget']), step=100.0)
    with col2:
        expense_budget = st.number_input("支出预算 (¥)", min_value=0.0, value=float(current_budget['expense_budget']), step=100.0)
        
    if st.button("保存预算"):
        db.set_budget(month_input, income_budget, expense_budget)
        st.success(f"已保存 {month_input} 的预算设置！")
